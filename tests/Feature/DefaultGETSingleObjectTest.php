...
test('Test request', function ($id, $request, $object) {
    ...
    expect($response->status())->toEqual(200)
        ->and($response->dto())->toBeInstanceOf($object::class);
...
test('Test request with SELECT query parameters', function ($id, $request, $object) {
    ...
    expect($response->status())->toEqual(200)
        ->and($response->json())->toHaveKeys(['name', 'id'])->toHaveLength(2)
        ->and($response->dto())->toBeInstanceOf($object::class);
})->depends('id', 'request', 'object');
...
test('Test GetWarehouse with invalid TOP query parameters', function ($id, $request) {
    $queryParameters = new SageODataBuilder();
    $queryParameters->top(-2);
...
test('Test request with METADATA query parameters', function ($id, $request, $object) {
    $queryParameters = new SageODataBuilder();
